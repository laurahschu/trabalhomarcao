...
def enviar_dados_e_assinatura(nome_sonda):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as cliente_socket:
        cliente_socket.connect(('127.0.0.1', 8080))
        cliente_socket.send(dados)  # Mantenha os dados como bytes
        cliente_socket.send(assinatura)  # Mantenha a assinatura como bytes
...
def lidar_com_conexao(cliente_socket):
    with cliente_socket:
        try:
            opcao = cliente_socket.recv(1).decode('utf-8')  # Especificar a codificação UTF-8
            if opcao == '2':
                nome_sonda = input("Nome da sonda: ")
                enviar_chave_publica(nome_sonda)
            elif opcao == '5':
                dados = cliente_socket.recv(4096).decode('utf-8')  # Especificar a codificação UTF-8
                assinatura = cliente_socket.recv(256)  # Manter a assinatura como bytes
                nome_arquivo = input("Nome do arquivo de dados: ")
                if verificar_assinatura(dados.encode('utf-8'), assinatura):  # Codificar os dados como UTF-8 antes de verificar a assinatura
                    print("Arquivo recebido com sucesso e a assinatura é válida.")
                else:
                    print("Arquivo inválido.")
        except Exception as e:
            print("Ocorreu um erro durante a comunicação com o cliente:", str(e))
